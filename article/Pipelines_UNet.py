...
class UNet(nn.Module):
    def __init__(self, input_dim=(X.shape[-1]), output_dim=(y.shape[-1])):
        ...
        self.l1 = nn.Linear(input_dim, input_dim//3)
        self.l2 = nn.Linear(input_dim//3, input_dim//5)
        self.l3 = nn.Linear(input_dim//5, input_dim//8)
        self.l4 = nn.Linear(input_dim//8, input_dim//5)
        self.l5 = nn.Linear(input_dim//5, input_dim//3)
        self.l6 = nn.Linear(input_dim//3, input_dim)
    ...
    def forward(self, x, x_guess):
        x = self.dropout(x)
        x0 = self.activation(self.l1(x))
        x1 = self.activation(self.l2(x0))
        x2 = self.activation(self.l3(x1))
        x3 = self.activation(self.l4(x2))+x1
        x4 = self.activation(self.l5(x3))+x0
        x5 = self.l6(x4)
        return self.alpha*x_guess + self.beta*x + self.gamma*x5.squeeze()
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(train)):
    ...
    with torch.no_grad():
        ...
        del model
        del pipe_lm
    scores["UNet"] = val_losses

    pd.DataFrame(scores).to_csv(f'scores_UNet{k}.csv')
    k += 1
