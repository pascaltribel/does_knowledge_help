...
class CNN(nn.Module):
    ...
    def forward(self, x, x_guess):
        return self.alpha*x_guess + self.beta*x + self.gamma*self.features(x).squeeze()
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(train)):
    ...
    with torch.no_grad():
        ...
        del model
        del pipe_lm
    scores["CNN1d"] = val_losses

    pd.DataFrame(scores).to_csv(f'scores_CNN{k}.csv')
    k += 1
pd.DataFrame(scores).to_csv('results_cnn1d_div_2_to_ps.csv')
